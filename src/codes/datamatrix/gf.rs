//! GF(256) arithmetic and Reed–Solomon codes as used by Data Matrix ECC 200.
//!
//! The field is GF(2^8) with primitive polynomial `x^8 + x^5 + x^3 + x^2 + 1`
//! (`0x12D`) and generator element `α = 2`, matching ISO/IEC 16022. This is a
//! **different** field from QR (`0x11D`); the two are intentionally kept separate.
//!
//! Data Matrix Reed–Solomon uses a generator polynomial whose first root is `α^1`
...
mod tests {
    ...
    /// Independent reference vector: the widely-cited ISO/IEC 16022 worked example.
    /// The digits "123456" encode (ASCII digit-pairs) to data codewords
    /// 142, 164, 186; the 10×10 ECC 200 symbol appends 5 error codewords which the
    /// standard gives as 114, 25, 5, 88, 102. This pins the field polynomial (0x12D)
    /// and the generator base (α^1) end to end.
    #[test]
