//! Static PDF417 low-level pattern tables from ISO/IEC 15438 Annex A.
//!
//! [`CODEWORD_PATTERNS`] gives, for each of the three row clusters (0, 3, 6)
//! and each codeword value `0..=928`, the 17-module bar/space pattern used to
//! render that codeword on a row of the corresponding cluster. A set bit is a
//! dark module; bit 16 (`0x10000`) is the leftmost module. Every pattern is 17
//! modules wide with exactly four bars and four spaces, beginning with a bar.
//!
//! The clusters repeat every three rows: row `y` uses cluster index `y % 3`.
//! These values are the codeword table of Annex A of ISO/IEC 15438; the same
//! table is reproduced in the (Apache-2.0) ZXing project.

/// The start pattern (`81111113`), 17 modules, rendered at the left of every row.
pub const START_PATTERN: u32 = 0x1fea8;
/// The stop pattern (`711311121`), 18 modules, rendered at the right of every row.
pub const STOP_PATTERN: u32 = 0x3fa29;
/// Width in modules of the [`START_PATTERN`].
pub const START_WIDTH: usize = 17;
/// Width in modules of the [`STOP_PATTERN`].
pub const STOP_WIDTH: usize = 18;
/// Width in modules of a single codeword symbol character.
pub const CODEWORD_WIDTH: usize = 17;

/// Row-cluster pattern table: `[cluster_index][codeword] -> 17-bit pattern`.
/// `cluster_index` is `row % 3` and selects clusters 0, 3 and 6 respectively.
pub const CODEWORD_PATTERNS: [[u32; 929]; 3] = [
    // cluster index 0 (row % 3 == 0)
    [
