...
//!     reproduce them bit-for-bit. The M3 bitmaps come from `zint` instead: segno
//!     zero-fills the spare M1/M3 data codewords where ISO/IEC 18004 §7.4.10 (and
//!     zint) pad with 0xEC / 0x11, keeping only the final 4-bit codeword `0000`.
...
fn m3_byte_l() {
    check(
        ...
        15,
        "111111101010101100000101111011101110100010101101110100001100101110100001001100000100000001111111101011100000000000111110111110010100000010010011110111100111100110111011110111100101110010011010100000001000110010110100011011101",
    );
}
...
fn m3_numeric_m() {
    check(
        ...
        15,
        "111111101010101100000101010111101110100101011101110100011010101110101000111100000100110101111111101001011000000001010110100000111100000001111101001010100111011111011010111111111010100111001111100010001111010111111100000000000",
    );
    check(
        Segment::alphanumeric(b"AB12".to_vec()),
        meta(MicroVersion::M3, MicroEcLevel::M, 2),
        15,
        "111111101010101100000100000100101110100100101101110100010100101110100101101100000101001100111111101101110000000000111010100011001010000000101101011000100001001101110010001110011100111001101000111011100011010101111010101001100",
    );
}
